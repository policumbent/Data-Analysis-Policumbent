...
if bikeChoice == 3
    ...
    %Bike Parameters Cerberus
    %Aerodynamics
    L = 2.80; %[m] Length of the fairing in the direction of flow
    Cx_vec = [0.02135,0.01965,0.0187,0.0177,0.01685,0.0164,0.016,0.0158,0.01575,0.0158,0.0159];
    V_vec = [30,40,50,60,70,80,90,100,105,110,115]/3.6;


    %Inertia
    mv = 30; %[kg] Mass of vehicle excluding wheels
    mr = 68; %[kg] Mass of rider, shoes, clothing, etc.
    mw = 1.5; %[kg] Mass of the single wheel
    m = mv+mr+mw;
    ...
    crr_2 = 0.000038; %Linear coefficient rolling resistance
    %tau_tot = 0.175; %Rapporto di trasmissione totale
    R_wheel = 0.23157; %[m] Raggio ruota effettivo
    R_g = 0.25; %[m] Raggio ruota geometrico
    me = mr+mv+((mw*(R_g^2))*3/2)/(R_wheel^2); %[kg] Equivalent mass
    n_start = 2;
...
open model_simulink_Cerb
sim model_simulink_Cerb
